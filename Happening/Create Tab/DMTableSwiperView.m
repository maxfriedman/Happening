...
- (id)initWithTableViews:(NSArray *)tableViewsArr tableFrame:(CGRect)tableFrame inViewController:(UIViewController *)viewController {
    self = [super initWithFrame:tableFrame]; //CGRectMake(0, 0, tableFrame.size.width + 4, tableFrame.size.height)];
    if (self) {
        if (tableViewsArr != nil) {
            [self setupSwiperWithTableViews:tableViewsArr inViewController:viewController];
        }
    }
    return self;
}

- (void)setupSwiperWithTableViews:(NSArray *)tableViewsArr inViewController:(UIViewController *)viewController {
    ...
    int i = 0;
    for (UITableView *tblView in tableViewsArr) {
