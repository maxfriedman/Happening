...
#define SYSTEM_VERSION_LESS_THAN_OR_EQUAL_TO(v)     ([[[UIDevic/Users/Max/Desktop/Happening/Happening/Discover Tab/ExternalProfileTVC.me currentDevice] systemVersion] compare:v options:NSNumericSearch] != NSOrderedDescending)

@interface ExternalProfileTVC () <EventTVCDelegate, UITableViewDataSource, UITableViewDelegate>
...
@synthesize nameLabel, detailLabel, profilePicImageView, settingsButton, containerView, profPicView, scoreButton, eventsArray, createdEventsArray, createdSections, createdSortedDays, pastEventsArray;
...
- (void)viewDidLoad {
    ...
    [self.backButton setHitTestEdgeInsets:(UIEdgeInsetsMake(-10, -10, -10, -10))];
    ...
    if (!user) {
        ...
        [userQuery getObjectInBackgroundWithId:self.userID block:^(PFObject *ob, NSError *error) {
            ...
            if (!error) {
                ...
                if (user[@"createdCount"] != nil) {
                    [createdButton setTitle:[NSString stringWithFormat:@"%@", [user[@"createdCount"] stringValue]] forState:UIControlStateNormal];
                } else {
                    user[@"createdCount"] = @0;
                    ...
                    [createdEventsQuery countObjectsInBackgroundWithBlock:^(int count, NSError *error) {
                        ...
                        if (!error) {
                            ...
                            [createdButton setTitle:[NSString stringWithFormat:@"%d", count] forState:UIControlStateNormal];
                        }
                    }];
                ...
                if (user[@"eventCount"] != nil) {
                    [eventsButton setTitle:[NSString stringWithFormat:@"%@", [user[@"eventCount"] stringValue]] forState:UIControlStateNormal];
                } else {
                    user[@"eventCount"] = @0;
                    ...
                    [swipedRightEventQuery countObjectsInBackgroundWithBlock:^(int count, NSError *error) {
                        ...
                        if (!error) {
                            ...
                            [eventsButton setTitle:[NSString stringWithFormat:@"%d", count] forState:UIControlStateNormal];
               ...
               /* if (user[@"friendCount"] != nil) {
                    NSLog(@"MADE IT &&");
                    [friendsButton setTitle:[NSString stringWithFormat:@"%@", [user[@"friendCount"] stringValue]] forState:UIControlStateNormal];
                } else*/ if (user[@"friends"] != nil) {
                    NSArray *friends = user[@"friends"];
                    [friendsButton setTitle:[NSString stringWithFormat:@"%lu", friends.count] forState:UIControlStateNormal];
                } else {
                ...
                if (user[@"score"] == nil) {
                    ...
                    [scoreButton setTitle:[NSString stringWithFormat:@"%@", [user[@"score"] stringValue]] forState:UIControlStateNormal];
                }
                
                if ([scoreButton.currentTitle isEqualToString:@"0"])
                    [scoreButton removeFromSuperview];
                if ([friendsButton.currentTitle isEqualToString:@"0"])
                    friendsButton.enabled = NO;
                if ([createdButton.currentTitle isEqualToString:@"0"])
                    createdButton.enabled = NO;
                ...
                scoreLabel.text = [user[@"score"] stringValue];
                scoreLabel.alpha = 0;
                ...
                xButton.center = CGPointMake(self.backButton.center.x + 5, scoreButton.center.y);
    ...
    [swipesQuery findObjectsInBackgroundWithBlock:^(NSArray *swipes, NSError *error){
        ...
        if (!error) {
            ...
            for (int i = 0; i < orderedObjects.count; i++) {
                ...
                if ([object[@"FBObjectID"] isEqualToString:user[@"FBObjectID"]]) {
            ...
            for (int i = 0; i < orderedObjects.count; i++) {
                ...
                if (![swipe[@"FBObjectID"] isEqualToString:user[@"FBObjectID"]] && swipe[@"FBObjectID"] != nil) {
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    if (tableVersion == 1) eventDate = [self.createdSortedDays objectAtIndex:section];
    else eventDate = [self.sortedDays objectAtIndex:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (showTimeline) {
        ...
        cell.messageLabel.text = [cell.messageLabel.text stringByReplacingOccurrencesOfString:@"You are" withString:[NSString stringWithFormat:@"%@ is", firstName]];
        cell.messageLabel.text = [cell.messageLabel.text stringByReplacingOccurrencesOfString:@"You" withString:firstName];
...
- (void)loadFriends {
    ...
    NSArray *friends = user[@"friends"];
    if (friends == nil) friends = [NSArray array];
